...
class Cliff(DistributionalEnv):
    ...
    def __init__(self,  nb_col=5, nb_line=4, reward_step=0, reward_end=10, reward_fall=-10, right_proba=0.7, retry=True):
    ...
    def _sto_transition(self, pos, action):
        ...
        x, y = pos
        trans = [[self.wrong_proba, pair_sum((x,y), action_to_dx(act), (0,0), (self.nb_col-1, self.nb_line-1)), self.reward_step] for act in list(Action)]
        trans[action][0] = self.right_proba

        #Could be cleaner
        if x==0 and y==0:                      #bottom left corner
            trans[Action.Right.value] = [trans[Action.Right.value][0], ((0,0) if self.retry else trans[Action.Right.value][1]), self.reward_fall]
            return trans
        elif 0 < x < self.nb_col-1 and y==1:   #along the cliff
            trans[Action.Down.value] = [trans[Action.Down.value][0], ((0,0) if self.retry else trans[Action.Right.value][1]), self.reward_fall]
            return trans
        elif x==self.nb_col-1 and y==1:        #before the goal
            trans[Action.Down.value][2] = self.reward_end
            return trans
        elif x>0 and y==0:                      #Terminal states
            return [[1,(x,y),0]]
        else:
            return trans         
    ...
    def display_actions(self, action_array):
        ...
        for y in reversed(range(1,nb_line)):
            for x in range(nb_col):
                print(action_symbols[action_array[x,y]], end=" ")
            print("")
        
        #first row
        print("\033[1;34m"+action_symbols[action_array[0,0]], end=" ")
        print("\033[1;31m" + "X "*(nb_col-2), end="")
